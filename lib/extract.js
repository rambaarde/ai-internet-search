 ...
 */
function harvest(raw, terms, want) {
 ...
 * a no-op when no browser is installed.
 */
async function tryRender(source, terms, want, opts) {
  if (!opts.render) return null;
  const html = await renderPage(source.url, { timeoutMs: opts.renderTimeoutMs });
  if (!html) return null;
  const h = harvest(html, terms, want);
  if (!h.claims.length) return null;
  return {
    ...source, read: true, rendered: true, reason: '',
    bytes: Buffer.byteLength(html), claims: h.claims,
 ...
 * @param {string[]} terms
 * @param {{claims?: number, render?: boolean, renderTimeoutMs?: number}} [opts]
 */
async function extractClaims(source, terms, opts = {}) {
  ...
  try {
    ...
    const h = harvest(raw, terms, want);
...
module.exports = { htmlToText, sentences, scoreSentence, extractClaims, readSources, stackOverflowApi };
