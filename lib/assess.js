 ...
 *
 * `next[i]` is the follow-up query for `missingTerms[i]`: the gap term plus
 * the question's leading two covered terms as its subject. Short on purpose.
 * Appending the term to the whole query makes it longer, and a long query is
 * what returned nothing from the match-every-term engines in the first place.
 *
 * `next[i]` is null when that query would use every word of the question:
 * for a question of three terms or fewer, "deep research leaderboard" became
 * "research leaderboard deep", the same search reordered. The gap is still
 * reported in `missingTerms`; there is just no narrower search to suggest.
 *
 * @param {object[]} sources
 * @param {string[]} terms
 * @returns {{missingTerms: string[], next: (string|null)[], unread: string[]}}
 */
function gaps(sources, terms) {
  ...
  // A direct URL identifies the source to inspect; its owner/repository slug
  // is not a claim the page is required to repeat verbatim.
  const missing = sources.some((s) => s.direct) ? [] : terms.filter((t) => !covered.has(t));
  const lead = (covered.size ? terms.filter((t) => covered.has(t)) : terms).slice(0, 2);
  const all = new Set(terms);
  const next = missing.map((t) => {
    const q = new Set([...lead, t]);
    return q.size >= all.size ? null : [...q].join(' ');
  });
  const unread = sources.filter((s) => !s.read).map((s) => `${s.host} (${s.reason})`);
  return { missingTerms: missing, next, unread };
}
