...
/**
 * Build the full query decision from a chosen kind. Factored out from
 * decideQuery so both evaluators — the regex heuristic here and, when it is
 * enabled, the Jev classifier in jev.js — produce the exact same downstream
 * shape and the same provider fan-out. Only who chose the value differs, and
 * `method` records which one did.
 *
 * @param {'definition'|'engineering'|'academic'} value
 * @param {Record<string, number>} probabilities
 * @param {number} confidence
 * @param {'deterministic'|'jev'} [method]
 */
function planForKind(value, probabilities, confidence, method = 'deterministic') {
  ...
  const stamp = (c) => (method === 'deterministic' ? c : { ...c, method });
  return {
    queryKind: stamp(choice('query_kind', value, probabilities, confidence)),
    handler: stamp(choice('handler', handler, { reference_sources: handler === 'reference_sources' ? 0.8 : 0.1, engineering_sources: handler === 'engineering_sources' ? 0.8 : 0.1, academic_sources: handler === 'academic_sources' ? 0.8 : 0.1 }, 0.8)),
    providerKinds,
    fanOut: { parallel: true, questions: ['query_kind', 'handler'], providers: providerKinds },
    sourceStrategy: stamp(choice('source_strategy', value === 'academic' ? 'papers_and_practitioners' : value === 'engineering' ? 'docs_and_practitioners' : 'reference_first', {
...
function decideQuery(question, kinds = []) {
  ...
  return planForKind(value, probabilities, probabilities[value]);
}
...
function decideResearch(state) {
  ...
  const authoritative = hasAuthoritativeEvidence(opened);
  if (state.plan) {
    value = 'inspect_plan';
  ...
  } else if (state.directOnly || conflicts.length || missing.length || state.certainty?.level === 'low' || !authoritative) {
...
module.exports = { choice, score, noul, uncertaintyBand, planForKind, decideQuery, decideResearch, consistencyCheck, hasAuthoritativeEvidence };
