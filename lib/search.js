...
const { webSearchProvider, marginaliaProvider } = require('./providers/websearch');
const { htmlToText } = require('./extract');
...
/** Return the first explicit web URL in a question, without accepting punctuation around it. */
function directUrl(q) {
  const match = String(q).match(URL_TOKEN)?.[0];
...
/** Searchable words from a URL path, used when the URL is the whole question. */
...
/** A user-provided URL is a candidate itself; provider discovery is not required. */
function directCandidate(question) {
  const url = directUrl(question);
  if (!url) return null;
  let title = url;
  try {
    const u = new URL(url);
    let path = u.pathname;
    try { path = decodeURIComponent(path); } catch { /* keep the URL-encoded path */ }
    path = path.replace(/^\/+|\/+$/g, '').replace(/\//g, '/');
    title = path || u.hostname;
  } catch { /* directUrl already validated it */ }
  return { url, title, via: 'direct', direct: true, why: 'direct URL requested' };
}

/** Whether a prompt contains only a URL and generic inspection wording. */
function directOnly(question) {
  if (!directUrl(question)) return false;
  const residual = String(question)
    .replace(URL_TOKEN, ' ')
    .replace(/\b(?:research|this|source|url|read|look|at|please|check|review|summari[sz]e)\b/gi, ' ')
    .replace(/[\s:;,!?-]+/g, '')
    .trim();
  return !residual;
}
...
const PROVIDERS = [
  ...
  },
  {
    name: 'github',
    kinds: ['engineering'],
    // Bug reports and their diagnoses live in issue trackers, which no other
    // key-free provider indexes: a "why does X fail" question came back with
    // no sources while the matching tailscale/tailscale issue existed. Keyless
    // search allows 10 requests/min; a GITHUB_TOKEN in the environment raises
    // that to 30, and is sent only to api.github.com.
    async run(q) {
      const auth = process.env.GITHUB_TOKEN ? { authorization: 'Bearer ' + process.env.GITHUB_TOKEN } : {};
      const j = await getJson('https://api.github.com/search/issues?per_page=10&q=' + encodeURIComponent(q), auth);
      return (j?.items || []).map((i) => ({ url: i.html_url, title: i.title || '', via: 'github' }));
    },
  },
  {
    name: 'stackexchange',
    kinds: ['engineering'],
    // Practitioner Q&A. Keyless, about 300 requests/day per IP.
    // ponytail: Stack Overflow only; add serverfault/superuser calls if ops
    // questions keep coming back empty.
    async run(q) {
      const j = await getJson(
        'https://api.stackexchange.com/2.3/search/advanced?pagesize=10&order=desc&sort=relevance&site=stackoverflow&q=' +
          encodeURIComponent(q)
      );
      // The API returns titles HTML-escaped ("What&#39;s the risk").
      return (j?.items || []).map((i) => ({ url: i.link, title: htmlToText(i.title || ''), via: 'stackexchange' }));
    },
  },
  // Optional keyed general-web search (Brave / Tavily / Serper / Google CSE) and
  // the keyless, opt-in Marginalia. Both are eligible for every kind and no-op
  // until enabled, so they add general-web recall the key-free providers
  // cannot -- without ever becoming required.
  webSearchProvider,
  marginaliaProvider,
];
...
function keywords(q, langs) {
  ...
  const withoutUrls = text.replace(URL_TOKEN, ' ');
  const stripped = (withoutUrls + ' ' + urls.map(urlWords).join(' ')).replace(CJK_FRAMING, ' ');
...
/**
 * How many leading terms a widened query keeps.
 *
 * Hacker News, Stack Exchange and GitHub match every term, so a long query
 * returns nothing: measured on a 9-term question, 0 hits from each, while its
 * first 3 terms returned 9-12 that still passed the full-term relevance
 * filter. Leading terms, not "most distinctive" ones, because order carries
 * the phrase (see keywords()) and the subject of a question comes first.
 */
const WIDE_TERMS = 3;
 ...
 *
 * A provider that answers the full query with nothing is asked once more with
 * the leading WIDE_TERMS terms ("start wide, then narrow"). Its results are
 * still filtered against every term of the question, so widening adds recall
 * without admitting off-topic pages. Providers that did return results are
 * never re-asked, which keeps rate-limited ones (GitHub: 10/min) in budget.
 *
 * @param {string} question
 * @param {{kinds?: string[], langs?: string[]}} [opts]
 * @returns {Promise<{candidates: object[], languages: string[], providers: string[], failed: string[],
 *   widened: {query: string, providers: string[]} | null}>}
 */
async function findCandidates(question, opts = {}) {
  const direct = directCandidate(question);
  if (direct) {
    return { candidates: [direct], languages: languagesFor(question), providers: ['direct'], failed: [], widened: null };
  }
  ...
  const wide = terms.length > WIDE_TERMS ? terms.slice(0, WIDE_TERMS).join(' ') : '';
  const chosen = PROVIDERS.filter((p) => p.kinds.some((k) => kinds.includes(k)));
  const settled = await Promise.all(
    chosen.map(async (p) => {
      try {
        let raw = await p.run(query, { langs });
        let widened = false;
        // ponytail: getJson reports an unreachable provider as [], so one that
        // is down is retried once here (worst case 2x TIMEOUT_MS, in parallel
        // with the others). Make getJson throw if that latency starts to matter.
        if (!raw.length && wide) {
          raw = await p.run(wide, { langs });
          widened = true;
        }
        return { name: p.name, widened, out: raw.filter((c) => looksRelevant(c.title, terms)) };
      } catch {
        return { name: p.name, widened: false, out: null };
      }
    })
  );
  ...
  const widenedBy = settled.filter((s) => s.widened && s.out && s.out.length).map((s) => s.name);
  return {
    candidates,
    languages: langs,
    providers: settled.filter((s) => s.out && s.out.length).map((s) => s.name),
    failed: settled.filter((s) => !s.out).map((s) => s.name),
    // Named only when the wide query actually contributed a candidate, so the
    // reader can tell which results came from fewer words than they typed.
    widened: widenedBy.length ? { query: wide, providers: widenedBy } : null,
  };
}

module.exports = { findCandidates, kindsFor, keywords, looksRelevant, languagesFor, fold, directUrl, directCandidate, directOnly, PROVIDERS };
