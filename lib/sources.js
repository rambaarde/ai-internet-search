 ...
 *
 * A vendor engineering blog is secondary in general, but primary about the
 * vendor's own product: Anthropic's post on Anthropic's research system is
 * the first-hand account, and grading it "secondhand" understated a correct
 * answer. That holds when the question names the vendor, or when the caller
 * gave the URL directly (the page itself is then the subject).
 *
 * @param {string} url
 * @param {{terms?: string[], direct?: boolean}} [ctx] the question's terms, and whether the URL was given directly
 * @returns {{tier: number, why: string, host: string}}
 */
function gradeSource(url, ctx = {}) {
  let host = '';
  let path = url;
  try {
    const u = new URL(url);
  ...
  for (const r of RULES) {
    if (!r.test(host, path)) continue;
    // Brand is the label before the TLD: research.google.com -> google.
    const brand = host.split('.').slice(-2)[0];
    if (r.why === 'vendor engineering blog' && (ctx.direct || (ctx.terms || []).includes(brand))) {
      return { tier: 1, why: 'vendor on its own product', host };
    }
    return { tier: r.tier, why: r.why, host };
  }
 ...
 * @param {string[]} [terms]
 * @returns {{score: number, breakdown: {authority: number, relevance: number, freshness: number}}}
 */
function scoreSource(candidate, terms = []) {
  const tier = candidate.tier ?? gradeSource(candidate.url, { terms }).tier;
  ...
  const relevance = normalizedTerms.length ? hits / normalizedTerms.length : 0.5;
  ...
  return { score, breakdown: { authority: Number(authority.toFixed(3)), relevance: Number(relevance.toFixed(3)), freshness: Number(age.toFixed(3)) } };
}
 ...
 * @param {{limit?: number, perHost?: number, terms?: string[]}} [opts]
 */
function triage(candidates, opts = {}) {
  ...
  return candidates
    .map((c) => {
      const g = gradeSource(c.url, { terms, direct: c.direct });
      return { ...c, ...g, ...scoreSource({ ...c, ...g }, terms) };
    })
